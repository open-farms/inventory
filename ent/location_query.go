...
func (lq *LocationQuery) QueryVehicle() *VehicleQuery {
	query := &VehicleQuery{config: lq.config}
...
func (lq *LocationQuery) Clone() *LocationQuery {
	...
	return &LocationQuery{
		...
		withVehicle: lq.withVehicle.Clone(),
...
func (lq *LocationQuery) WithVehicle(opts ...func(*VehicleQuery)) *LocationQuery {
	query := &VehicleQuery{config: lq.config}
	for _, opt := range opts {
		opt(query)
	}
	lq.withVehicle = query
	return lq
}
...
func (lq *LocationQuery) sqlAll(ctx context.Context) ([]*Location, error) {
	...
	if query := lq.withVehicle; query != nil {
		...
		query.Where(predicate.Vehicle(func(s *sql.Selector) {
			s.Where(sql.InValues(location.VehicleColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.location_vehicle
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "location_vehicle" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "location_vehicle" returned %v for node %v`, *fk, n.ID)
			}
